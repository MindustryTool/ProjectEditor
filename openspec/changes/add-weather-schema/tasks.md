...
- [x] 1.3 Define `rainWeatherObjectSchema` with `RainWeather` fields (own Java defaults): `stroke` (0.75), `splashTimeScale` (22), plus shared-name fields with RainWeather defaults `yspeed` (5), `xspeed` (1.5), `sizeMin` (8), `sizeMax` (40), `color` ("7a95eaff") and matching ParticleWeather defaults for `padding` (16)/`density` (1200); exclude `splashes`
