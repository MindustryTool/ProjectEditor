...
**`RainWeather.java`:**
`stroke` (0.75), `splashTimeScale` (22), `liquid` (LiquidFieldSchema(context), added in the RainWeather ClassMap entry since it needs context). Shared-name fields differ from ParticleWeather only where RainWeather.java overrides them: `yspeed` 5, `xspeed` 1.5, `sizeMin` 8, `sizeMax` 40, `color` "7a95eaff" (RainWeather.java:11,14); `padding` (16) and `density` (1200) match ParticleWeather's defaults. `splashes` is excluded — it is auto-loaded from the atlas in `load()` (RainWeather.java:24-27), not meaningful user config.
...
- [Shared field names (yspeed, xspeed, sizeMin, sizeMax, color) have different Java defaults in ParticleWeather vs RainWeather] → Per-class schemas each carry their own Java defaults (ParticleWeather: yspeed -2, xspeed 0.25, sizeMin 2.4, sizeMax 12; RainWeather: yspeed 5, xspeed 1.5, sizeMin 8, sizeMax 40, color "7a95eaff"), so defaults never conflate across classes. `padding`/`density` are identical (16/1200) in both classes.
