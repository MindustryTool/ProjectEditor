...
The system SHALL provide a `WriteQueue` class that manages debounced file writes to disk via a single flush queue, replacing per-hook `setTimeout` calls.
...
#### Scenario: Write error reported via callback
- **WHEN** a disk write fails
- **THEN** the error SHALL be reported via an `onError` callback passed at construction, not thrown
