...
The system SHALL display entry counts for each state filter option (all, translated, untranslated, invalid).
...
- **WHEN** a row has `existsInBundle = false`
- **THEN** its value input SHALL NOT be disabled
- **THEN** typing in the input SHALL queue a write that adds the key to the bundle
