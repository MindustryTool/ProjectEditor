...
The system SHALL provide a mechanism to detect a schema's "special type" (e.g., "color", "research", "effect") from
`metadata({ type: "..." })` annotations rather than from an external registry.
...
- **WHEN** a schema without a `metadata({ type: "..." })` annotation is passed
