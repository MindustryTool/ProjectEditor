...
export class Tokenizer {
  ...
  readMultilineString(): string {
    ...
    let current = "";
    while (this.pos < this.input.length) {
      if (this.input[this.pos] === "'" && this.pos + 2 < this.input.length && this.input[this.pos + 1] === "'" && this.input[this.pos + 2] === "'") {
        this.advance();
        this.advance();
        this.advance();
  ...
  expect(type: TokenType): Token {
    ...
    if (token.type !== type) {
      this.error(
        HJSONErrorCode.UnexpectedToken,
  ...
  private scan(): Token {
    ...
    if (this.pos >= this.input.length) {
      return token("EOF", "", this.row, this.col, this.pos);
    }
    ...
    if (ch === "'" && this.pos + 2 < this.input.length && this.input[this.pos + 1] === "'" && this.input[this.pos + 2] === "'") {
      ...
      return token("string", value, startRow, startCol, startIdx);
    }
    ...
    if (ch === "-" || ch === "+" || isDigit(ch)) {
      ...
      if (numStr.length > 0 && numStr !== "-" && numStr !== "+") {
        return token("number", numStr, startRow, startCol, startIdx);
      }
      this.pos = savedPos;
      this.row = startRow;
      this.col = startCol;
    ...
    switch (ch) {
      case "{": {
        this.advance();
        return token("{", "{", startRow, startCol, startIdx);
      }
      case "}": {
        this.advance();
        return token("}", "}", startRow, startCol, startIdx);
    ...
    if (isIdentStart(ch)) {
      ...
      if (word === "true" || word === "false") {
        return token("boolean", word, startRow, startCol, startIdx);
      }
      if (word === "null") {
        return token("null", word, startRow, startCol, startIdx);
      }
...
function token(type: TokenType, value: string, row: number, col: number, index: number): Token {
  return { type, value, row, col, index };
}
