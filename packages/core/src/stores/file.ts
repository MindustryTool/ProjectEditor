...
import type { EventBus, ProjectEventMap } from "../index.js";
...
export function isDirty(entry: FileEntry | undefined): boolean {
	if (!entry) return false;
	return entry.currentVersion !== entry.savedVersion;
}

export function isError(entry: FileEntry | undefined): boolean {
	if (!entry) return false;
...
export function getEntry(projectId: string, path: string) {
	const key = cacheKey(projectId, path);
	return useFileStore.getState().fileContents[key];
}
...
function buildFiles(): Record<string, FileEntry> {
