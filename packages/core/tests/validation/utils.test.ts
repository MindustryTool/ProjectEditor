...
import { metadata } from "@project/schema";
...
describe("findUnknownProperties", () => {
	...
	describe("array schema", () => {
		...
		it("handles arrays nested inside objects", () => {
			...
			const value = {
				items: [{ name: "foo", extra: 1 }, { name: "bar" }, { name: "baz", other: true }],
			};
			const result = findUnknownProperties(schema, value);
		...
		it("handles nested arrays (array of arrays)", () => {
			...
			const value = [[{ id: 1 }, { id: 2, extra: "x" }], [{ id: 3 }]];
	...
	describe("wrapper unwrapping", () => {
		it("unwraps nullish → intersect effect-like schema and detects unknown properties inside", () => {
			const schema = v.object({
				effect: v.nullish(
					v.intersect([v.object({ type: v.picklist(["ParticleEffect"]) }), v.object({ type: v.literal("ParticleEffect") })]),
				),
			});
			const value = {
				effect: {
		...
		it("handles intersect where options have different entries (merged)", () => {
			const schema = v.object({
				cfg: v.intersect([v.object({ a: v.string() }), v.object({ b: v.number() })]),
			});
		...
		it("does not report properties that exist in any intersected option", () => {
			const schema = v.object({
				cfg: v.intersect([v.object({ a: v.string(), common: v.number() }), v.object({ b: v.number(), common: v.number() })]),
			});
	...
	describe("lazy schema", () => {
		...
		it("resolves lazy schemas inside arrays", () => {
			const schema = v.object({
				effects: v.array(v.lazy(() => v.object({ name: v.string() }))),
			});
			const value = {
				effects: [{ name: "a", extra: true }, { name: "b" }],
			};
	...
	describe("path construction", () => {
		...
		it("handles mixed dot and bracket paths", () => {
			...
			const value = {
				matrix: [[{ val: 1 }, { val: 2 }], [{ val: 3, extra: true }]],
			};
