...
export type BasePosition = {
	x: { value: number; path: string };
	y: { value: number; path: string };
};

export type SpritePositionData = {
	type: "sprite";
	name: string;
	path: string;
	mirror: boolean;
...
export type EnginePositionData = {
	type: "engine";
	radius: { value: number; path: string };
	rotation: { value: number; path: string };
	position: BasePosition;
};
...
export type HitboxPositionData = {
	type: "hitbox";
	position: BasePosition;
	size: { value: number; path: string };
};
...
export function collectHitboxPosition(raw: Record<string, unknown>): HitboxPositionData[] {
	return [
		{
			type: "hitbox",
			position: {
				x: { value: 0, path: "hitSize" },
				y: { value: 0, path: "hitSize" },
			},
			size: { value: (raw.hitSize as number) ?? 6, path: "hitSize" },
		},
	];
}
