...
export const ColorPicker = ({ value, defaultValue = "#000000", onChange, className, ...props }: ColorPickerProps) => {
	...
	const selectedColor = safeColor(value || defaultValue);
	const defaultColor = safeColor(defaultValue);
	...
	useEffect(() => {
		if (value) {
			...
			setLightness((prev) => (Math.abs(prev - color.lightness()) > 1 ? color.lightness() : prev));
			setAlpha((prev) => (Math.abs(prev - color.alpha() * 100) > 1 ? color.alpha() * 100 : prev));
		}
	}, [value]);
	...
	useEffect(() => {
		if (onChange) {
			...
			const newHex = color.alpha() < 1 ? color.hexa() : color.hex();
			if (newHex !== value) {
				onChange(newHex);
			}
		}
...
export const ColorPickerHue = ({ className, ...props }: ColorPickerHueProps) => {
	...
	return (
		<Slider.Root
			...
			max={360}
			onValueChange={(vals) => setHue(vals[0] ?? 0)}
			step={1}
			value={[hue]}
			{...(props as any)}
		>
...
export const ColorPickerAlpha = ({ className, ...props }: ColorPickerAlphaProps) => {
	...
	return (
		<Slider.Root
			className={cn("relative flex h-4 w-full touch-none", className)}
			max={100}
			onValueChange={(vals) => setAlpha(vals[0] ?? 100)}
			step={1}
			value={[alpha]}
			{...(props as any)}
		>
