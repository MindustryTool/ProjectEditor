...
export function ShootPreview({
	...
	const weaponX = sprite.weaponPosition.x.value;
	const weaponY = sprite.weaponPosition.y.value;
	const offsetX = sprite.position.x.value;
	const offsetY = sprite.position.y.value;
	...
	return (
		...
		>
			<div className="relative flex flex-col items-center justify-center gap-1 p-8" onClick={() => scrollTo(sprite.position.x.path)}>
				<span className="absolute top-1 left-1 text-xs text-muted-foreground">[shoot]</span>
				{sprite.mirror && (
				...
				<div className="text-muted-foreground text-sm">
					weapon=({weaponX}, {weaponY}) offset=({offsetX}, {offsetY})
				</div>
			</div>
